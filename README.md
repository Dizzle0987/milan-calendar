...
Il campo `broadcast_it` viene aggiunto automaticamente dai fallback stagionali configurati in `data/broadcast_sources.json`. L'elenco seguente descrive la stagione 2026/27, non regole permanenti:
...
Ogni fallback possiede un intervallo di stagioni: scaduto l'intervallo torna `Da definire` invece di ereditare automaticamente il broadcaster precedente. Per le partite ufficiali UEFA, FIFA o intercontinentali non ancora iniziate viene inoltre richiesta una conferma della **singola partita**: possedere i diritti generali di un torneo non basta.

Le fonti controllate sono configurate in `data/broadcast_sources.json`. La verifica avanzata viene eseguita **soltanto sulla prossima partita ufficiale UEFA, FIFA o intercontinentale**: Serie A, Serie B, Coppa Italia, Supercoppa Italiana e amichevoli mantengono integralmente il proprio percorso già collaudato. Il generatore identifica automaticamente il Paese dell'avversaria, verifica l'Italia e consulta fonti internazionali ordinate per tier configurabili. I tier stabiliscono soltanto l'ordine: una guida strutturata JSON-LD può scoprire broadcaster nuovi o rinominati fuori dal registro. Un broadcaster scoperto senza prova affidabile di gratuità resta `UNKNOWN` e non viene pubblicato come alternativa gratuita.

Ogni conferma riceve un punteggio verificabile: 100 per una pagina ufficiale della singola partita, 95 per un EPG ufficiale, 90 per due guide indipendenti concordi e 70 per una sola guida. Una guida sostenuta da diritti **temporalmente compatibili** arriva a 75; i soli diritti stagionali arrivano a 60 e producono `POSSIBLE_FREE`, mai una conferma. Diritti privi di stagione/date non vengono promossi a prova valida. `CONFIRMED_FREE` e `CONFIRMED_PAY` richiedono sempre evidenza della singola partita e confidenza almeno 80. La priorità italiana è un attributo stagionale della fonte, non un controllo sul nome del broadcaster.

La verifica richiede le due squadre (con alias), la data locale e, per le guide che lo supportano, il calcio d'inizio convertito nel timezone della fonte. I palinsesti strutturati devono inoltre coprire la partita, escludendo anteprime, notiziari, highlights e differite. Nei dati di debug `source_schedule` dimostra la programmazione, mentre `watch_url` è esclusivamente un player/pagina live su un dominio ufficiale autorizzato; `broadcast_free` e `web_stream_free` distinguono la TV in chiaro dallo streaming web. Se manca un link ufficiale verificabile non viene creato alcun pulsante.

I palinsesti vengono interrogati entro il loro orizzonte configurato, normalmente 21 giorni; le pagine ufficiali dedicate possono essere cercate prima. Evidenze e punteggi sono salvati in `broadcast_candidates`, mentre le sole opzioni pubblicabili finiscono in `broadcast_options` dentro `data/events.json`. Le chiavi di riuso includono competizione, stagione, Paese, identità della fixture e data: una conferma 2026/27 non può contaminare il 2027/28. Ogni fonte è indipendente; l'ultima conferma della stessa fixture viene conservata soltanto durante un errore temporaneo, non quando una fonte scade o viene rimossa.
