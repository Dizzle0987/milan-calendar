...
Le note visibili degli eventi sono intenzionalmente compatte: mostrano competizione, casa/trasferta, orario di Roma, stadio, emittente, fonte dell'orario e classifica Milan (solo Serie A). Gli URL non affollano la descrizione: rimangono nel campo tecnico `URL` dell'evento e, insieme a tutte le fonti TV/orario, in `data/events.json`.
