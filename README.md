...
Le fonti ufficiali controllate sono configurate in `data/broadcast_sources.json`. Questa verifica avanzata viene eseguita **soltanto** sulle partite ufficiali UEFA, FIFA o intercontinentali: Serie A, Serie B, Coppa Italia, Supercoppa Italiana e amichevoli mantengono il proprio percorso già collaudato. Per le coppe europee il generatore controlla la pagina ufficiale della singola partita su Programmi Sky, l'API strutturata di TV8, il palinsesto ufficiale ServusTV e i siti dei broadcaster del Paese dell'avversaria. Il registro 2026/27 copre le attuali avversarie del Milan in Portogallo, Austria, Regno Unito, Ungheria, Grecia, Bulgaria e Armenia; può essere esteso dopo i nuovi sorteggi senza cambiare il generatore.

Una copertura viene pubblicata solo quando nella stessa pagina o voce di palinsesto compaiono entrambe le squadre, la data locale esatta e un'indicazione di diretta TV/streaming. Per i palinsesti strutturati viene inoltre richiesto che l'intervallo copra la partita: questo esclude anteprime, notiziari, highlights e differite. L'inizio del programma TV viene conservato separatamente dal calcio d'inizio e mostrato sempre convertito in `Europe/Rome`. Le opzioni italiane ed estere sono ordinate dando precedenza a quelle gratuite, poi a quelle incluse e infine a quelle a pagamento; all'estero vengono mostrate al massimo tre alternative, con paese, lingua, costo, piattaforma e link ufficiale. Prime Video non viene mai descritto come gratuito e non vengono forniti suggerimenti per aggirare i geoblocchi.

I palinsesti vengono interrogati per le gare nei 21 giorni successivi; le pagine ufficiali dedicate di Sky possono essere cercate prima. I risultati verificati sono salvati in `broadcast_options` dentro `data/events.json`. Ogni emittente è indipendente: timeout, blocco geografico, risposta 403/404 o cambio di formato di una fonte non interrompono l'aggiornamento. L'ultima conferma valida viene conservata e l'errore resta soltanto nei dati di debug. Un evento passato non viene riscritto. Se nessun palinsesto della singola partita è ancora disponibile compare `Da confermare`, senza deduzioni basate sui soli diritti generali o sulla nazionalità dell'avversaria.
