...
Le fonti controllate sono configurate in `data/broadcast_sources.json`. La verifica avanzata viene eseguita **soltanto sulla prossima partita ufficiale UEFA, FIFA o intercontinentale**: Serie A, Serie B, Coppa Italia, Supercoppa Italiana e amichevoli mantengono integralmente il proprio percorso già collaudato. Il generatore identifica automaticamente il Paese dell'avversaria, verifica l'Italia e consulta un registro finito di broadcaster e guide internazionali, ordinato per tier. Austria, Svizzera, Germania, Belgio, Azerbaijan, Turchia, Albania e Serbia sono Tier 1; Kosovo, Croazia, Bosnia-Erzegovina, Moldova, Malta, Kazakhstan, Georgia e Cipro sono Tier 2. Le fonti gratuite configurate fuori da questi tier restano interrogabili: i tier ordinano il lavoro, non costituiscono una lista bloccante.

Ogni conferma riceve un punteggio verificabile: 100 per una pagina ufficiale della singola partita, 95 per un EPG ufficiale, 90 per due guide indipendenti concordi e 70 per una sola guida. Una guida sostenuta soltanto dalla pagina generica dei diritti arriva a 75; i soli diritti arrivano a 60 e producono `POSSIBLE_FREE`, mai una conferma. `CONFIRMED_FREE` e `CONFIRMED_PAY` richiedono sempre evidenza della singola partita e confidenza almeno 80. In Italia Sky/NOW resta l'opzione principale quando è confermata; un canale gratuito italiano è separato e le alternative estere gratuite sono aggiuntive.
